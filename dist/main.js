...
const buttons = [
    ...
    new UIButton({ x: 800, y: 600 }, { width: 150, height: 30 }, 'Tạo ảnh mới', () => {
        var _a;
        const src = (_a = prompt('URL ảnh:', '')) === null || _a === void 0 ? void 0 : _a.trim();
        if (!src)
            return;
...
inputManager.subscribe((x, y) => { if ((moveX || moveY) && sprite.contains(x, y))
    scoreLabel.add(1); });
// Render
const renderManager = new RenderManager(ctx, sprite, buttons, infoLabel, scoreLabel, () => moveX, () => moveY, () => deltaX, () => deltaY);
renderManager.start();
